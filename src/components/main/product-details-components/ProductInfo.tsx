  ...
  return (
    <div className="space-y-6 mt-4 lg:mt-0  dark:text-white">
      ...
      <div className="flex items-center justify-between  dark:text-white">
        <Link
          href={`/products/all-products/${productData?.seller?.UserCompanyInfo?.slug}`}
        >
          <p className="text-orange-600 font-bold text-sm  dark:text-white">
            {productData?.seller?.UserCompanyInfo?.shopName}
          </p>
        </Link>
      ...
      <p className="text-sm text-gray-600  dark:text-white">
        ...
        <span
          className={
            inStock ? "text-green-500 font-medium" : "text-red-500 font-medium"
          }
        >
          {inStock
            ...
            : hasAnyStock
            ? translate(
                "অন্যান্য ভ্যারিয়েন্ট স্টকে আছে",
                "Other variants in stock"
              )
      ...
      {/* Variation Options */}
      {VariationType?.filter((variation) => variation).map((variation) => (
        <div
          key={variation?.id}
